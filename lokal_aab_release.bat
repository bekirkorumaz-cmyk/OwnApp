...
call gradlew.bat :app:bundleRelease -PnewArchEnabled=true "-PreactNativeArchitectures=armeabi-v7a,arm64-v8a,x86,x86_64" --console=plain --no-daemon --max-workers=2

if errorlevel 1 (
  echo.
  echo Lokal release AAB olusturulamadi. Hata mesaji yukarida.
  pause
  exit /b %errorlevel%
)
